...
class Program:
    def __init__(self, cfg):
        ...
        if ann:
            ...
            a = json.load(open(ann if os.path.isabs(ann) else os.path.join(base, ann)))
            for key in ('names', 'comments', 'symbols', 'line_comments'):
                merged = dict(cfg.get(key, {}))
                merged.update(a.get(key, {}))
                cfg[key] = merged
        self.cfg = cfg
        adf = Adf(cfg['adf'])
    ...
    # ---- CPU time -------------------------------------------------------------
    def block_costs(self, cycles):
        """68000 cycles of every basic block, keyed by the block's first address.

        The port adds a block's cost when it enters the block; the reference
        emulator does the same when it executes the block's first instruction,
        so both see identical CPU time. Costs come from Musashi's 68000 table
        plus the per-register/per-shift parts of movem and shifts."""
        ctl = ('bra', 'bsr', 'jsr', 'jmp', 'rts', 'rte', 'rtr', 'illegal')
        leaders = set(a for a in self.entries if a in self.code) | set(a for a in self.code_consts if a in self.code)
        for a in self.code:
            ins = self.insns[a]
            is_ctl = ins.mn in ctl or (ins.cc is not None and (ins.mn.startswith('b') or ins.mn.startswith('db')))
            if ins.target is not None and ins.target in self.code:
                leaders.add(ins.target)
            if is_ctl and a + ins.size in self.code:
                leaders.add(a + ins.size)

        def cost(ins):
            c = cycles[ins.raw]
            if ins.mn == 'movem':
                mask = [o for o in ins.ops if o.kind == 'reglist'][0].val
                c += bin(mask).count('1') * (8 if ins.sz == 'l' else 4)
            elif ins.mn in ('lsl', 'lsr', 'asl', 'asr', 'rol', 'ror', 'roxl', 'roxr') and len(ins.ops) == 2 \
                    and ins.ops[0].kind == 'imm':
                c += 2 * ins.ops[0].val
            return c
        out = {}
        for l in leaders:
...
class Emitter:
    ...
    def emit_insn(self, fe, body, ins, live):
        ...
        if mn in ('move', 'movea'):
            ...
            if mn == 'movea':
                if src.kind == 'imm' and sz == 'l' and src.val in self.p.symbols:
                    v = self.p.symbols[src.val]
                if sz == 'w':
                    v = 'sxw(%s)' % v
    ...
    def emit_function(self, e, body, tails):
        ...
        for idx, a in enumerate(order):
            ...
            if a in self.labels:
                out.append('L_%06X:' % a)
            if a in self.p.costs:
                out.append('    tick(%d);' % self.p.costs[a])
            txt = repr(ins)[7:]
            ls = lines_by_addr[a]
            if self.trace:
...
def main():
    ...
    p.build_functions()
    p.liveness()
    cyc = open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'm68000_cycles.bin'), 'rb').read()
    p.block_costs(cyc)
    ...
    hdr = ['// Generated by tools/m68k2cpp.py from the original 68000 code. Do not edit by hand;',
           ...
           '#pragma once', '#include <cstdint>', '', 'namespace game {']
    for e in funcs:
        hdr.append('void %s();' % em.fname(e))
    for a, nm in sorted(p.natives.items()):
        hdr.append('void %s();  // native replacement of $%06X' % (nm, a))
    hdr.append('')
    hdr.append('// named addresses of the original program (variables, tables, constants)')
    for a, nm in sorted(p.symbols.items()):
        hdr.append('constexpr uint32_t %s = 0x%X;' % (nm, a))
    ...
    # CPU time of each basic block (shared with the reference emulator)
    c = ['// Generated by tools/m68k2cpp.py. 68000 cycles per basic block (first address, cycles).',
         '#include <cstddef>', '#include <cstdint>', '', 'namespace game {', '',
         'extern const uint32_t kBlockCycles[][2];', 'extern const size_t kBlockCycleCount;',
         'const uint32_t kBlockCycles[][2] = {']
    for a in sorted(p.costs):
        c.append('    {0x%X, %d},' % (a, p.costs[a]))
    c += ['};', 'const size_t kBlockCycleCount = sizeof(kBlockCycles) / sizeof(kBlockCycles[0]);', '',
          '}  // namespace game']
    if not trace_out:
        open(os.path.join(out_dir, 'gen_cycles.cpp'), 'w').write('\n'.join(c) + '\n')
    # dispatcher
